...
var ajaxCart = {
	...
	if (jQuery('.container_24').width() > 450) {
				if ($picture.size())
					$picture.css({'position': 'absolute', 'width':210, 'height':268 , 'top': pictureOffsetOriginal.top, 'left': pictureOffsetOriginal.left});
	}
	if (jQuery('.container_24').width() < 450) {
				if ($picture.size())
					$picture.css({'position': 'absolute', 'width':99, 'height':126 , 'top': pictureOffsetOriginal.top, 'left': pictureOffsetOriginal.left});
	}
		...
		$(jsonData.products).each(function(){
			...
			if (this.id != undefined)
			{
				...
				if ($('#cart_block dt#cart_block_product_'+ domIdProduct ).length == 0)
				{
					...
					content += '<a style="float:left!important; width:70px!important;display:inline-block!important;" href="' + this.link + '" title="' + this.name.substring(0, 12) + '"><img style="float:left!important; width:70px!important;display:inline-block!important;" src="' + this.id_image + '" alt="' + this.name +'"></a>';
					
					
					var name = (this.name.length > 25 ? this.name.substring(0, 25) + '...' : this.name);
					content += '<a style="float:right!important;display:inline-block!important;width:124px!important; padding-right:20px; margin-top:-3px" class="extra" href="' + this.link + '" title="' + this.name + '">' + name + '</a>';
	
					content += '<span style="margin-left:10px;" class="quantity-formated"><span class="quantity">' + this.quantity + '</span>x</span>';
