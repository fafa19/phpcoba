...
{if isset($p) AND $p}
	...
	{if $start!=$stop}
		...
		{if $p != 1}
			...
			<li id="pagination_previous"><a class="button" {$no_follow_text} href="{$link->goPage($requestPage, $p_previous)}"><span></span>{l s='Previous'}</a></li>
		{else}
			<li id="pagination_previous" class="disabled"><span class="button">{l s='Previous'}</span></li>
		{/if}
		{if $start==3}
			<li class="pag-top"><a class="bordercolor" {$no_follow_text}  href="{$link->goPage($requestPage, 1)}">1</a></li>
			<li class="pag-top"><a class="bordercolor" {$no_follow_text}  href="{$link->goPage($requestPage, 2)}">2</a></li>
		{/if}
		{if $start==2}
			<li class="pag-top"><a class="bordercolor" {$no_follow_text}  href="{$link->goPage($requestPage, 1)}">1</a></li>
		{/if}
		...
		{section name=pagination start=$start loop=$stop+1 step=1}
			{if $p == $smarty.section.pagination.index}
				<li class="current pag-top"><span>{$p|escape:'htmlall':'UTF-8'}</span></li>
			{else}
				<li class="pag-top"><a class="bordercolor" {$no_follow_text} href="{$link->goPage($requestPage, $smarty.section.pagination.index)}">{$smarty.section.pagination.index|escape:'htmlall':'UTF-8'}</a></li>
			{/if}
		{/section}
		{if $pages_nb>$stop+2}
			<li class="truncate pag-top">...</li>
			<li class="pag-top"><a class="bordercolor" href="{$link->goPage($requestPage, $pages_nb)}">{$pages_nb|intval}</a></li>
		{/if}
		{if $pages_nb==$stop+1}
			<li class="pag-top"><a class="bordercolor" href="{$link->goPage($requestPage, $pages_nb)}">{$pages_nb|intval}</a></li>
		{/if}
		{if $pages_nb==$stop+2}
			<li class="pag-top"><a class="bordercolor" href="{$link->goPage($requestPage, $pages_nb-1)}">{$pages_nb-1|intval}</a></li>
			<li class="pag-top"><a class="bordercolor" href="{$link->goPage($requestPage, $pages_nb)}">{$pages_nb|intval}</a></li>
		{/if}
		{if $pages_nb > 1 AND $p != $pages_nb}
			{assign var='p_next' value=$p+1}
			<li id="pagination_next"><a class="button" {$no_follow_text} href="{$link->goPage($requestPage, $p_next)}">{l s='Next'}<span></span></a></li>
		{else}
